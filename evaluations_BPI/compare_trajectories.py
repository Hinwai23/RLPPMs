...
class PPOGreedyHelper:
    def __init__(self, model_path: str = 'ppo_bpi_best.pth') -> None:
        self.model_path = model_path
        self.activity2idx = activity2idx
        self.env = BPIEnv(all_transitions, activity2idx, use_true_end_reward=True)
        state_shape = (29,)
...
class RainbowGreedyHelper:
    def __init__(self, model_path: str = 'rainbow_bpi_best.pth') -> None:
        self.model_path = model_path
        self.activity2idx = activity2idx
        self.idx2activity = {i: act for act, i in activity2idx.items()}
        self.env = BPIEnv(all_transitions, activity2idx, use_true_end_reward=True)
    ...
    def get_optimal_action(self, state_str: str) -> str | None:
        ...
        with torch.no_grad():
            ...
            logits = logits.squeeze(0)
            q_values = self._q_from_logits(logits)
            masked_q_values = q_values.clone()
            mask = torch.ones_like(q_values) * float('-inf')
            mask[valid_actions] = 0
            masked_q_values = masked_q_values + mask
...
def generate_action_sequence(start_state: str, helper) -> list[str]:
    ...
    for _ in range(max_steps):
        ...
        next_tuple = helper.env.transitions.get(current_state, {}).get(action, (None, None))
        next_state = next_tuple[0]
        
        if action == 'O_ACCEPTED' or next_state is None or next_state == 'END':
            break
