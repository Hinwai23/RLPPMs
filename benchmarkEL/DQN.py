...
from EL_env.csv_to_gym_EL import ELEnv, activity2idx, train_transitions, all_transitions, ActionMaskObsWrapper




def make_train_env():
    return ActionMaskObsWrapper(ELEnv(train_transitions, activity2idx, use_true_end_reward=True, reward_scale=0.001))

def make_eval_env():
    return ActionMaskObsWrapper(ELEnv(all_transitions, activity2idx, use_true_end_reward=True, reward_scale=0.001))



def save_best_fn(policy):
    torch.save(policy.state_dict(), 'dqn_el_best.pth')


if __name__ == "__main__":

    timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    log_dir = f"training_logs/DQN_EL_{timestamp}"
    ...
    optim_ = optim.Adam(net.parameters(), lr=5e-3)
    policy = DQNPolicy(
        model=net,
        optim=optim_,
