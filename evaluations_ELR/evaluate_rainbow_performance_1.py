...
class RainbowEvaluator:
    def __init__(self, model_path='rainbow_elr_best.pth'):
        """
        Initialize the Rainbow evaluator for ELR
        """
    ...
    def get_optimal_action(self, state_str):
        ...
        with torch.no_grad():
            ...
            q_values = torch.sum(probs * support.unsqueeze(0).unsqueeze(0), dim=-1)
    ...
    def calculate_offer_acceptance_rate(self, case_data):
        ...
        return 1.0 if final_reward > 0 else 0.0
    ...
    def print_results(self, results):
        print("=" * 80)
        print("RAINBOW EVALUATION RESULTS (ELR)")
        print("=" * 80)
...
def main():
    print("Initializing Rainbow evaluator (ELR)...")
    evaluator = RainbowEvaluator('rainbow_elr_best.pth')
